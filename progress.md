...
## 2026-08-10 — 首頁 Logo 對齊與呼吸

- 保持 Logo 圖片在黑色圓形視窗內正中央，並將整個 `.home-menu-panel`／Logo 視窗依最新畫面回饋下移 `70px`，對準頭盔金屬圓環中心。
- Logo 本體新增 4.8 秒的輕微縮放與冷光呼吸；使用減少動態偏好時停用動畫。
- 依專案規則只做靜態測試與建置，不執行瀏覽器／Playwright。

## 2026-08-10 — 第三部分 34–40 公尺中央空氣牆清除
...
## 2026-08-10 — 移除第三部分 34 公尺階梯

- 確認下沉篇 Part 3 的 34 公尺階梯是唯一的 `layerPortal`：`-10,34|-10,35`；它也是 29–40 公尺 T2 小區域唯一的 T2→T1 入口。
- 從生成器移除這個不穩定的階梯，並把該小區域統一為 T1，避免留下新的隱形層邊界；新增回歸測試，避免重新生成地圖後再次出現，其他章節不變。
- 地圖測試 29/29、完整 check 376/376 與 build 已通過；修正已提交並 push。依專案規則不執行瀏覽器／Playwright。
...
- Boss 3 `abyssal-throne` 在 row 109、右上側 column 17（`-37,109`）放置 1 個托里切利空間，同樣保留可通行水域與適度的角落距離。
- 地圖專項測試 27/27、完整 check 372/372 與 build 已通過；已提交並 push。依專案規則不執行瀏覽器／Playwright。

## 2026-08-10 — 修復第三部分 36–38 公尺隱形牆

- 將 `-7,37`、`-6,37`、`-6,38` 從錯誤的 blocked 地形開成水域，保留各自原本的 gravityLevel 與 T1 水層。
- 新增 36→37→38→39→40 公尺連續 Cell 路徑測試，避免只驗到 38 公尺下方繞路而漏掉上方隱形牆。
- 地圖專項測試 27/27 已通過；完整 check、build 與 push 待完成。依專案規則不執行瀏覽器／Playwright。

## 2026-08-10 — 第三部分增加托里切利空間

- 確認使用者所說的「脫離切力」是托里切利空間；第三部分從原本 3 個補給點增加到 5 個。
- 新增的兩個點放入第 28、61 列的 L-1 螢光／淺藍水域群，避免玩家長距離沒有氧氣回復空間。
- Part 3 地圖專項測試 26/26、完整 check 363/363 與 build 已通過；本次變更接著提交並 push。依專案規則不執行瀏覽器／Playwright。
...
- 三個下沉篇 Boss 房在入口封印事件後自動切換 `boss` 曲目；跨到下一部分時恢復 `normal` 曲目，手動音樂選單仍可覆寫目前模式。
- 靜態驗證完成：焦點測試 58/58、完整 `npm run check` 測試 359/359、`npm run build` 成功；依專案規則不執行瀏覽器／Playwright。
...
## 2026-08-10 — Skip Tutorial continues into the formal chapter

- `Skip Tutorial` 不再導回 `/home.html`；現在與完成 Tutorial 出口共用下沉篇第一部分的轉場，會打開既有故事投影片。
- Skip 對話文案同步說明會進入 Descent Part 1；Play 靜態測試 21/21、完整 `npm run check` 與 Vite production build 通過，依專案規則未執行瀏覽器／Playwright。
...
## 2026-08-10 — Formal developer depth teleport

- 正式遊戲的 `Ctrl + Alt + D` 開發者面板新增目前地圖的深度輸入與傳送按鈕；輸入公尺數後會實際更新玩家位置、鏡頭與深度 HUD，不是只改顯示文字。
