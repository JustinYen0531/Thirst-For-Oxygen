...
const messages = Object.freeze({
  en: Object.freeze({
    ...
    'mode.actionsAria': 'Editor controls',
    ...
    'map.official.heading': 'Playable Map', 'map.official.choose': 'Choose a playable map…', 'map.official.part1': 'Descent · Part 1', 'map.official.part2': 'Descent · Part 2', 'map.official.part3': 'Descent · Part 3',
    'map.official.load': 'Load Playable Map', 'map.official.save': 'Save & Apply to Game', 'map.official.help': 'Load the map currently used by Play. Saving overwrites that playable map when the editor is running locally.',
    'map.official.loading': 'Loading the playable map…', 'map.official.loaded': 'Loaded {name}. You can now edit it.', 'map.official.saving': 'Validating and saving the playable map…', 'map.official.saved': 'Saved {name}. Play now uses this version.',
    'map.official.error': 'Playable map operation failed: {message}', 'map.official.chooseFirst': 'Choose a playable map first.', 'map.official.invalidDocument': 'This file is not a valid game map.', 'map.official.loadFailed': 'Unable to load the playable map (HTTP {status}).', 'map.official.saveUnavailable': 'Direct saving is available through the local development server only.', 'map.official.invalid': 'This map has {count} structural error(s). Fix them before saving.',
    ...
    'dirty.unsaved': 'Not saved locally or exported as JSON.', 'dirty.saved': 'Saved in this browser.', 'dirty.officialLoaded': 'Playable map loaded. Changes have not been applied to the game yet.', 'dirty.officialSaved': 'Saved to the playable game map.',
  ...
  'zh-Hant': Object.freeze({
    ...
    'map.official.heading': '目前遊玩地圖', 'map.official.choose': '選擇要修改的遊玩地圖…', 'map.official.part1': '下沉篇・第一部分', 'map.official.part2': '下沉篇・第二部分', 'map.official.part3': '下沉篇・第三部分',
    'map.official.load': '載入遊玩地圖', 'map.official.save': '儲存並套用到遊戲', 'map.official.help': '載入 Play 目前使用的正式地圖；在本機啟動編輯器時，儲存會直接更新這張遊玩地圖。',
    'map.official.loading': '正在載入目前的遊玩地圖…', 'map.official.loaded': '已載入{name}，現在可以直接修改。', 'map.official.saving': '正在驗證並儲存遊玩地圖…', 'map.official.saved': '已儲存{name}，Play 現在會使用這個版本。',
    'map.official.error': '遊玩地圖操作失敗：{message}', 'map.official.chooseFirst': '請先選擇一張遊玩地圖。', 'map.official.invalidDocument': '這個檔案不是有效的遊戲地圖。', 'map.official.loadFailed': '無法載入遊玩地圖（HTTP {status}）。', 'map.official.saveUnavailable': '只有透過本機開發伺服器開啟時才能直接儲存。', 'map.official.invalid': '這張地圖有 {count} 個結構錯誤，修正後才能儲存。',
    ...
    'play.help': '物理測試：拖曳玩家並放開以彈射；氧氣以時間倒數，滿氧約 40 秒；靠近水草、珊瑚或潛壁鰓門後按 E 互動；R 重設。',
    ...
    'action.chooseAsset': '選擇此素材', 'action.viewUse': '查看素材用途', 'action.back': '返回', 'dirty.unsaved': '尚未儲存到本機或匯出 JSON。', 'dirty.saved': '已儲存到瀏覽器本機。', 'dirty.officialLoaded': '已載入遊玩地圖；修改尚未套用到遊戲。', 'dirty.officialSaved': '已儲存到正式遊玩地圖。', 'editor.ready': '編輯器已就緒；新地圖從空白 L0 水域開始，儲存後刷新會保留本機版本。', 'official.default': '官方預設：{value}', 'official.restore': '恢復官方預設',
...
const rawEnglish = Object.freeze({
  ...
  '潛壁鰓門：放在水域與不可通行牆面的交界。靠近後按 E 進入牆內，也必須在鰓門旁按 E 離開。': 'Wall-Gill Gate: place it between water and blocked terrain. Press E nearby to enter the wall, and press E at a gate to leave.',
  '潛壁鰓門只能放在水域與不可通行牆面的交界。': 'Wall-Gill Gates can only be placed on the boundary between water and blocked terrain.',
  ...
  '附近沒有可附著的水草。': 'No attachable seaweed nearby.', '已離開水草，重力重新生效。': 'Left the seaweed; gravity is active again.', '已附著邊緣水草：暫停重力並回復體力。': 'Attached to edge seaweed: gravity is paused and energy recovers.', '已附著水草：暫停重力並回復體力。': 'Attached to seaweed: gravity is paused and energy recovers.', '光合作用氣泡作用中，暫時無法彈射。': 'Photosynthesis bubble active: launching is temporarily disabled.',
  ...
  '潛壁鰓門：已潛入牆體；牆內仍會消耗氧氣，且無法攻擊。': 'Wall-Gill Gate: entered the wall. Oxygen keeps draining and attacks are disabled inside.',
  '潛壁鰓門：已離開牆體，回到可通行水域。': 'Wall-Gill Gate: left the wall and returned to passable water.',
  '牆體內側封閉：必須找到潛壁鰓門才能回到水域。': 'The wall is sealed from within. Find a Wall-Gill Gate to return to the water.',
  ...
  '邊緣沾黏：以底座貼在六角邊，優先朝可通行水域一側伸出。物理測試按 E 可附著或離開。': 'Edge attachment: anchors its base to the hex edge and extends toward passable water. Press E in the physics test to attach or leave.', '邊緣珊瑚：靠近後按 E，可獲得 2.5 秒隱形。': 'Edge coral: approach and press E to become invisible for 2.5 seconds.',
...
const dynamicRules = [
  ...
  [/^氧氣礦石：需要 (.+) m\/s 撞擊才會釋放氧氣。$/, ([, speed]) => `Oxygen ore requires an impact of ${speed} m/s to release oxygen.`],
  [/^托里切利空間：以 (.+) O₂\/s 回復氧氣。$/, ([, rate]) => `Torricelli space restores oxygen at ${rate} O₂/s.`],
  [/^光合作用氣泡：\+(.+) O₂，(.+) 秒免疫水域重力，(.+) 秒內無法彈射；氣泡已消失。$/, ([, amount, duration, lockDuration]) => `Photosynthesis bubble: +${amount} O₂, ${duration} seconds of gravity immunity, and launching disabled for ${lockDuration} seconds; the bubble is consumed.`],
